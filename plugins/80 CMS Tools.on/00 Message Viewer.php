<?php
	...
	$viewmessages.= $GLOBALS['MESSAGES']->pullquery('<div style="display: block; margin-top: 1em; font-size: 80%;">%k%</div>%v%', "SELECT * FROM __REGISTRY____pmessages WHERE value LIKE '#a.%' ORDER BY value DESC LIMIT 20;");
	$viewmessages.='<br/><h3>Last 20 Notifications:</h3>';
	$viewmessages.= $GLOBALS['MESSAGES']->pullquery('<div style="display: block; margin-top: 1em; font-size: 80%;">%k%</div>%v%', "SELECT * FROM __REGISTRY____pmessages WHERE value LIKE '#n.%' ORDER BY value DESC LIMIT 20;");
	$viewmessages.='<br/><h3>Last 20 Debugging Messages:</h3>';
	$viewmessages.= $GLOBALS['MESSAGES']->pullquery('<div style="display: block; margin-top: 1em; font-size: 80%;">%k%</div>%v%', "SELECT * FROM __REGISTRY____pmessages WHERE value LIKE '#d.%' ORDER BY value DESC LIMIT 20;");
