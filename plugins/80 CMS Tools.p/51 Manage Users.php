...
if (!isset($_SESSION['user_list_limit'])) {
	$_SESSION['user_list_limit'] = '';
}
...
}elseif(!isset($_SESSION['user_list_limit']) || $_SESSION['user_list_limit'] == ''){
...
if (!isset($_SESSION['user_list_filter'])) {
	$_SESSION['user_list_filter'] = '';
}
...
if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Edit User' && trim(stripslashes($_POST['u_uid'])) == null ){
	$this->messageAddAlert('You must select a user to edit.');
	$_POST['u_submit'] = null;
}

if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Delete User'){
...
if(isset($_POST['u_submit']) && ($_POST['u_submit'] == 'Save User Details' || $_POST['u_submit'] == 'Create User Account')){
	//i do need to preprocess the password
	//null is no change

	$autopass = false; // Initialize $autopass to false

	if(isset($_POST['u_autopass']) && $_POST['u_autopass'] == 'on'){
		$autopass = true;
		$password = null;
	}elseif(isset($_POST['u_pass1']) && isset($_POST['u_pass2']) && $_POST['u_pass1'] == $_POST['u_pass2'] && strlen($_POST['u_pass1']) >= 4){
	...
	if($GLOBALS['UGP']->user_put(Array(
		'id' => (isset($_POST['u_uid']) ? stripslashes($_POST['u_uid']) : null),
		'username' => (isset($_POST['u_username']) ? stripslashes($_POST['u_username']) : null),
		'name' => (isset($_POST['u_name']) ? stripslashes($_POST['u_name']) : null),
		'email' => (isset($_POST['u_email']) ? stripslashes($_POST['u_email']) : null),
		'status' => (isset($_POST['u_status']) ? stripslashes($_POST['u_status']) : null),
		'autopass' => $autopass,
		'password' => $password
	))){
	};

	if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Create User Account'){
		$_POST['u_submit'] = null;
	}
}

if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Add Membership'){
	$GLOBALS['UGP']->membership_add($_POST['u_uid'], $_POST['u_group']);
}

if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Remove Membership'){
	$GLOBALS['UGP']->membership_remove($_POST['g_mid']);
}

if(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Set User Value' && isset($_POST['a_id']) && $_POST['a_id'] != null){
...
if((isset($_POST['u_submit']) && $_POST['u_submit'] == 'Add User' && isset($_POST['u_username']) && trim(stripslashes($_POST['u_username'])) != null) || (isset($_POST['u_submit']) && $_POST['u_submit'] == 'Create User Account')){
	phylobyte::$breadcrumbs.=' &raquo; Add New User';
...
<form action="?'.$_SERVER['QUERY_STRING'].'" method="POST">
	<input type="hidden" name="u_initialusername" value="'.(isset($_POST['u_username']) ? $_POST['u_username'] : '').'" />
	<input type="hidden" name="u_uid" value="" />
	<label for="u_username">User Name</label><input type="text" name="u_username" value="'.(isset($_POST['u_username']) ? $_POST['u_username'] : '').'"/><br/>
	<label for="u_name">Nick Name</label><input type="text" name="u_name" value="'.(isset($_POST['u_fname']) ? $_POST['u_fname'] : '').'"/><br/>
	<label for="u_email">e-Mail Address</label><input type="text" name="u_email" value="'.(isset($_POST['u_email']) ? $_POST['u_email'] : '').'"/><br/>
	<hr/>
	...
	<label for="u_pass1">Password</label><input type="password" name="u_pass1" value="'.(isset($_POST['u_pass1']) ? $_POST['u_pass1'] : '').'"/><br/>
	<label for="u_pass2">Password (Again)</label><input type="password" name="u_pass2" value="'.(isset($_POST['u_pass2']) ? $_POST['u_pass2'] : '').'"/>
	</span>
...
}elseif(	(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Edit User') || (isset($_POST['u_submit']) && $_POST['u_submit'] == 'Add Membership') ||
			(isset($_POST['u_submit']) && $_POST['u_submit'] == 'Save User Details') || (isset($_POST['u_submit']) && $_POST['u_submit'] == 'Remove Membership')){

phylobyte::$breadcrumbs.=' &raquo; Edit User';

if(isset($_POST['u_uid']) && $_POST['u_uid'] == $_SESSION['loginid']){
...
$userExists = $GLOBALS['UGP']->user_get(isset($_POST['u_uid']) ? stripslashes($_POST['u_uid']) : null);

$userStatus = ''; // Initialize userStatus
if (!empty($userExists) && isset($userExists[0]['status'])) {
    $userStatus = $userExists[0]['status'];
}
...
switch($userStatus){
	...
	default:
		if(ctype_digit((string)$userStatus) || $userStatus == 'reserved'){
			$status = 'Reserved (No password changes allowed)';
		}else{
			$status = ucfirst((string)$userStatus);
		}
}
...
//list of existing groups
$groupList = $GLOBALS['UGP']->group_get(null, null, (isset($_POST['u_uid']) ? $_POST['u_uid'] : null), true);

$groupMemberships = $GLOBALS['UGP']->group_get(null, null, (isset($_POST['u_uid']) ? $_POST['u_uid'] : null));
...
}elseif((isset($_POST['u_submit']) && $_POST['u_submit'] == 'Edit Attributes') || (isset($_POST['u_submit']) && $_POST['u_submit'] == 'Set User Value')){

phylobyte::$breadcrumbs.=' &raquo; Edit Attributes';

$groupMemberships = $GLOBALS['UGP']->group_get(null, null, (isset($_POST['u_uid']) ? $_POST['u_uid'] : null));

foreach($groupMemberships as $groupMembership) {

	$attributes = $GLOBALS['UGP']->user_getattrs((isset($_POST['u_uid']) ? $_POST['u_uid'] : null), $groupMembership['id']);
	...
	if($attributes == false){
		...
		<fieldset>
			<legend>Edit User Attributes for Group "'.$groupMembership['name'].'"</legend>
		<form action="?'.$_SERVER['QUERY_STRING'].'" method="POST">
		<input type="hidden" name="u_uid" value="'.(isset($_POST['u_uid']) ? stripslashes($_POST['u_uid']) : '').'" />
			<table class="selTable">
				<tr>
...
$userList = $GLOBALS['UGP']->user_get(null, (isset($_SESSION['user_list_filter']) ? $_SESSION['user_list_filter'] : ''), (isset($_SESSION['user_list_limit']) ? $_SESSION['user_list_limit'] : ''));
...
<form action="?'.$_SERVER['QUERY_STRING'].'" method="POST">
	...
	<label for="u_filter">Filter Users</label><input type="text" name="u_filter" value="'.(isset($_SESSION['user_list_filter']) ? $_SESSION['user_list_filter'] : '').'"/><br/>
	<label for="u_limit">Limit of Users Shown</label><input type="text" name="u_limit" value="'.(isset($_SESSION['user_list_limit']) ? $_SESSION['user_list_limit'] : '').'"/><br/>
