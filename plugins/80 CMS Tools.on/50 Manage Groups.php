...
if( $_POST['g_action'] == 'edit' && ctype_digit($_POST['g_groupid']) ){
	...
	$attributesArray = $GLOBALS['UGP']->group_attributesGet($_POST['g_groupid'], '');

	$groupAttributesTemplate = '
	<tr id="g_attributes_row_%i%" class="table_row_normal">
	<td style="text-align: center;">
		<input type="radio" name="g_attributeid" value="%i%"
		onchange="changeLast(\'table_row_normal\', \'table_row_highlight\');changeClass(\'g_attributes_row_%i%\', \'table_row_normal\', \'table_row_highlight\');"
	...
	<form action="?'.$_SERVER['QUERY_STRING'].'" method="POST">
		...
		<input type="hidden" name="g_groupid" value="'.$_POST['g_groupid'].'" />
		...
		<table class="selTable">
			...
			'.$GLOBALS['UGP']->group_attributesFormat($attributesArray, $groupAttributesTemplate).'
		</table>
